...
import com.jayceebee.budgie.listadpters.CategoryDetailCursorAdapter;
...
public class CategoryDetailHome extends Activity {
    /** Called when the activity is first created. */
	
	IntentFilter mIntentFilter;
	CategoryDetailCursorAdapter cursorAdapter;
	Cursor cursor;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		String categoryString = getIntent().getExtras().getBundle("com.jayceebee.budgie.categorydetailname").getString("category_name");
		
		Log.d(BudgieApplication.TAG, "Received category name " + categoryString);
		
		this.setTitle(categoryString + " merchants");
		
		
		app = (BudgieApplication)getApplication();
    	
      	setContentView(R.layout.main_category_detail);
      	
      	mIntentFilter = new IntentFilter();
      	mIntentFilter.addAction(BudgieApplication.EVENT_UPDATE_MERCHANTS);
      	registerReceiver(mIntentReceiver, mIntentFilter);
      	      	        
      	ListView lv = (ListView) findViewById(R.id.CategoryDetailList);
      	      	
      	String [] from = new String []{BudgieDBAdapter.COLUMN_VIEW_MERCHANT_NAME, BudgieDBAdapter.COLUMN_VIEW_AMOUNT};
      	int [] to = new int []{R.id.merchantname, R.id.merchant_amount};
      	
      	cursor = ((BudgieApplication)this.getApplication()).getDbHelper().fetchAllMerchantsForCategory(categoryString);
      	cursorAdapter = new CategoryDetailCursorAdapter(this, R.layout.row_categorydetail, cursor, from, to);
