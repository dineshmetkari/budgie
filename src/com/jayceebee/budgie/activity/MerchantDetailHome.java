...
import com.jayceebee.budgie.listadpters.MerchantDetailCursorAdapter;
...
public class MerchantDetailHome extends Activity {
    /** Called when the activity is first created. */
	
	IntentFilter mIntentFilter;
	MerchantDetailCursorAdapter cursorAdapter;
	Cursor cursor;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		String merchString = getIntent().getExtras().getBundle("com.jayceebee.budgie.merchantdetailname").getString("merchant_name");
		
		Log.d(BudgieApplication.TAG, "Received merchant name " + merchString);
		
		this.setTitle(merchString + " transactions");
		
		
		app = (BudgieApplication)getApplication();
    	
      	setContentView(R.layout.main_merchant_detail);
      	
      	mIntentFilter = new IntentFilter();
      	mIntentFilter.addAction(BudgieApplication.EVENT_UPDATE_MERCHANTS);
      	registerReceiver(mIntentReceiver, mIntentFilter);
      	      	        
      	ListView lv = (ListView) findViewById(R.id.MerchantDetailList);
      	      	
      	String [] from = new String []{BudgieDBAdapter.COLUMN_VIEW_DATE, BudgieDBAdapter.COLUMN_VIEW_AMOUNT};
      	int [] to = new int []{R.id.transaction_date, R.id.transaction_amount};
      	
      	cursor = ((BudgieApplication)this.getApplication()).getDbHelper().fetchAllTransactionsForMerchant(merchString);
      	cursorAdapter = new MerchantDetailCursorAdapter(this, R.layout.row_merchantdetail, cursor, from, to);
